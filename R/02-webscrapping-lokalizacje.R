...
mar = 0.10 # bufor do przeszukania w stopniach od zadanych wspolrzednych
# zakladam, ze 22-23 km to jest minimalna wartosc majaca sens

#2021-06-05" "2021-06-24"
library(dplyr)
library(lubridate)
lokalizacje$data2 = format(lokalizacje$hail_date, "%m-%d")
#lokalizacje = filter(lokalizacje, data2 %in% c("06-05", "06-24"))

plot(lokalizacje$longitude, lokalizacje$latitude)
map("world", add = TRUE)
sort(unique(lokalizacje$hail_date))

lok = split(lokalizacje, lokalizacje$id)
...
for (day_no in 1:length(lok)) {
  ...
  min_lon = min(lok[[day_no]]$longitude - mar)
  min_lat = min(lok[[day_no]]$latitude - mar)
  max_lon = max(lok[[day_no]]$longitude + mar)
  max_lat = max(lok[[day_no]]$latitude + mar)
